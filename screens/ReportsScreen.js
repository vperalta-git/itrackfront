...
const getDateRange = (period) => {
  ...
  switch (period) {
    ...
    case 'all-time':
      ...
      // Set endDate far in the future so future-dated records are included
...
export default function ReportsScreen() {
  ...
  const fetchStats = useCallback(async () => {
    ...
    try {
      ...
      if (data && typeof data === 'object') {
        ...
        const releasedVehicles = data.releasedVehicles || data.completedAllocations || 0;
        ...
        setStats({
          ...
          totalReleasedVehicles: releasedVehicles,
  ...
  const fetchReleaseHistory = useCallback(async () => {
    ...
    try {
      console.log('📡 Fetching release history from /api/release-history...');
      const response = await fetch(buildApiUrl('/api/release-history'));
      const text = await response.text();

      if (!text) {
        console.warn('Release history: empty response');
        setReleaseHistory([]);
        return;
      }
      ...
      // If server returned HTML (e.g., 404 page), skip
      if (text.trim().startsWith('<')) {
        console.warn('Release history: HTML response (not JSON)');
        setReleaseHistory([]);
        return;
      }
      ...
      console.log('📊 Release history response:', data.success, 'Count:', data.data?.length || 0);
      ...
      console.log('🎯 Mapped release history:', mapped.length, 'items');

          ...
          >
            ...
            <View style={styles.modernStatsContainer}>
              ...
              <StatCard
                ...
                value={vehicles.filter(v => v.requestedProcesses?.length > 0).length}
                icon="🔧"
                color="#8B0000"
            ...
            <View style={styles.section}>
              {loading ? (
                ...
                filterBySearch(vehicles, ["unitId", "unitName", "assignedDriver"])
                  .filter(vehicle => 
                    vehicle.requestedProcesses && 
                    vehicle.requestedProcesses.length > 0
                  )
                  .map((vehicle, index) => {
                    const completedProcesses = Object.keys(vehicle.processStatus || {}).filter(
                      key => vehicle.processStatus[key] === true
                    ).length;
                    const totalProcesses = vehicle.requestedProcesses?.length || 0;
                    ...
                    return (
                      <View key={index} style={styles.vehiclePrepCard}>
                        ...
                        <View style={styles.processListSection}>
                          ...
                          {(vehicle.requestedProcesses || []).map(processId => {
